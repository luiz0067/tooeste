<?php
	...
	if (!function_exists("DAOquery")) {
	    function DAOquery($sql,$params,$select_execute,$caption,$limitParams=null){
			$data=array();
			$titles=array();
			$result=array();
	        if ($sql!="")
    		try {
    		    ...
    		    $Statement__ = null;
    		    //conect::getInstance()->setAttribute( PDO::ATTR_EMULATE_PREPARES, false );				
    			$Statement__ = conect::getInstance()->prepare($sql);
			    if ((is_array($limitParams))&&(count($limitParams)>0))
    				if (
    			...
    			foreach ($params as $key_param => $value){
    			     if(is_array($params[$key_param])){
    			         if( substr(array_keys($params[$key_param])[0],0,1)!=":")//not bind start operator whith :
	    ...
	    function DAOquerySelect($table,$fields_params,$joins,$conditionsParams,$groups,$having,$orders,$limitParams){
			...
			if ((is_array($limitParams))&&(count($limitParams)>0))
				if (
					...
					$offset=intval($row_count)*intval($page);
	...
	if (!function_exists("DAOquerySelectById")) {
	    function DAOquerySelectById($table,$fields_params,$joins,$id){
			...
			$limitParams=["offset"=>0,"row_count"=>1];
